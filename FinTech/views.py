...
def dashboard(request):
    if request.user.is_authenticated:
        ...
        for i in temp_pre_10days_list:
            pre_10days_list.append(f"{i.day}{i.strftime('%b')}")
...
def edit_expense(request,id):
    if request.user.is_authenticated:
        ...
        if request.method=="POST":
            ...
            date=request.POST['date']
            note=request.POST.get('note')
            category = get_object_or_404(ExpenseCategory, user=request.user.id,id=int(category_id))
...
def remove_expense_category(request,id):
    if request.user.is_authenticated:
        get_object_or_404(ExpenseCategory, user=request.user.id,id=id).delete()
...
def remove_income_category(request,id):
    if request.user.is_authenticated:
        get_object_or_404(IncomeCategory, user=request.user.id,id=id).delete()
        print("Category Deleted")
        return redirect("/settings")
    else:
        return redirect("/")
